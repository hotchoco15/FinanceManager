...
namespace Services
{
	public class IncomeService : IIncomeService
	{
		...
		// 페이징하여 보여주기
		public async Task<PagingResponse<IncomeResponse>> GetPages(List<IncomeResponse> list, int page, int pageSize)
		{
			var skipResult = (page - 1) * pageSize;

			var pagenatedIncomes = list.Skip(skipResult).Take(pageSize).ToList();
			...
			var response = new PagingResponse<IncomeResponse>();

			response.Data = pagenatedIncomes;
